...
mb::test::login()
{
  ${CCURL} "${Header}" http://${INSECURE_SERVER}/login \
    -d'{"username":"root","password":"miniweb1234"}' | grep -Po 'token[" :]+\K[^"]+'
}
...
mb::test::user()
{
  ...
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users \
    -d'{"username":"colin","password":"miniweb1234","nickname":"belm","email":"nosbelm@qq.com","phone":"1818888xxxx"}' > /dev/null
...
mb::test::post()
{
  ...
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/v1/users \
    -d'{"username":"posttest","password":"miniweb1234","nickname":"belm","email":"nosbelm@qq.com","phone":"1818888xxxx"}' > /dev/null

  # 2. posttest 登录 miniweb
  tokenStr=`${CCURL} "${Header}" http://${INSECURE_SERVER}/login -d'{"username":"posttest","password":"miniweb1234"}' | grep -Po 'token[" :]+\K[^"]+'`
